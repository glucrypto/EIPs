...
Propses a standard API from agreeable tokens (AT) within smart contracts. An agreeable token is a bilateral opt-in token bound to consenting accounts. ATs don't implement a canonical interface for transfers. This EIP defines basic functionality to mint, revoke and track ATs.
...
There exists a need for a standard that contracts and addresses can use to understand, track and create a mutual agreement between two parties. With a standard approach to agreements we can facilitate cross community

Special care was taken to respect the communities right to enter agreements consensually, and
